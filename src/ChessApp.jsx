...
export class MoveTable extends React.Component {
  ...
  render = () => {
    ...
    if (data.length == 0) return <div style={{ textAlign: "center" }}>Histórico vazio</div>
    return <div id="moveTable" className={styles.gameTable} style={{ textAlign: "center" }}> <ReactTable pageSize={data.length} showPagination={false} data={data} columns={moveColumns} getTdProps={this.onRowClick} /></div>
  }
}
